...
export const getPets = async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      `SELECT 
        mascotas.*, 
        municipios.name AS municipality, 
        razas.name AS race_name,
        categorias.name AS category_name, 
        mascotas.id AS id 
      FROM mascotas 
      LEFT JOIN municipios ON mascotas.id_municipality = municipios.id 
      LEFT JOIN razas ON mascotas.id_race = razas.id
      LEFT JOIN categorias ON mascotas.id_category = categorias.id`
    );
...
export const createPets = async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      'INSERT INTO mascotas (name, id_race, age, sterilized, gender, image, description, background, location, id_municipality, vaccines, phone_admin, id_category) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
...
export const updatePet = async (req, res) => {
  try {
    ...
    const [oldPet] = await pool.query('SELECT * FROM mascotas WHERE id = ?', [id]);
    ...
    const [result] = await pool.query('UPDATE mascotas SET ? WHERE id = ?', [data, id]);
