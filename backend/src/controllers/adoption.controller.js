...
export const getAdoptions = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(
            ...
            JOIN mascotas AS p ON a.id_pet = p.id 
            JOIN usuarios u ON a.id_user = u.id 
            LEFT JOIN municipios m ON p.id_municipality = m.id 
            LEFT JOIN razas r ON p.id_race = r.id 
            LEFT JOIN categorias c ON p.id_category = c.id`  // Une con la tabla de categorías
        );
...
export const getMyAdoption = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(
            ...
            LEFT JOIN municipios m ON p.id_municipality = m.id 
            LEFT JOIN razas r ON p.id_race = r.id 
            LEFT JOIN categorias c ON p.id_category = c.id
...
export const createAdoption = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(
            "INSERT INTO adopciones (id_user, id_pet, location, description_user) VALUES (?, ?, ?, ?)",
            [id_user, id_pet, location, description_user]
        );

        // Actualiza el estado de la mascota a 'En proceso'
        const [pet] = await pool.query(
            "UPDATE mascotas SET state = 'En proceso' WHERE id = ?",
            [id_pet]
        );
...
export const updateAdoption = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(
            'UPDATE adopciones SET id_user = ?, id_pet = ?, location = ? WHERE id = ?',
...
export const deleteAdoptionByUser = async (req, res) => {
    try {
        ...
        const [r] = await pool.query('UPDATE mascotas SET state = ? WHERE id = ?', [1, id_pet]);

        // Elimina la adopción del usuario y la mascota
        const [result] = await pool.query('DELETE FROM adopciones WHERE id_user = ? AND id_pet = ?', [id_user, id_pet]);
...
export const acceptAdoption = async (req, res) => {
    ...
    try {
        ...
        const [result] = await pool.query(
            'UPDATE adopciones SET state = 2, description_admin=? WHERE id_user = ? AND id_pet = ?',
        ...
        const [petResult] = await pool.query('UPDATE mascotas SET state = 3 WHERE id = ?', [id_pet]);
...
export const rejectAdoption = async (req, res) => {
    ...
    try {
        ...
        const [result] = await pool.query('UPDATE adopciones SET state = 3, description_admin=? WHERE id_user = ? AND id_pet = ?', [description_admin, id, id_pet]);
        ...
        // Actualiza el estado de la mascota a 'Sin adoptar'
        const [petResult] = await pool.query('UPDATE mascotas SET state = 1 WHERE id = ?', [id_pet]);
        if (petResult.affectedRows === 0) {
            await pool.query('ROLLBACK');
            return res.status(404).json({ message: 'No se pudo actualizar el estado de la mascota, registro no encontrado' });
        }
